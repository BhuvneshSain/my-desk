...
export default function InwardRegister() {
  ...
  const [doc, setDoc] = useState(null); // { name, type, size, data }
  ...
  const [items, setItems] = useState(() => getFromLocalStorage(LOCAL_STORAGE_KEYS.INWARD, []));
  ...
  const filtered = useMemo(() => {
    ...
    const list = items || [];
    if (!q) return list;
    return list.filter(it => `${it.fileNo} ${it.fromOffice} ${(it.subject||'')} ${(it.note||'')}`.toLowerCase().includes(q));
  }, [items, query]);
  ...
  const onFileChange = (e) => {
    ...
    reader.onload = () => {
      setDoc({ name: f.name, type: f.type, size: f.size, data: reader.result });
    };
  ...
  const save = async () => {
    if (!validate()) return;
    setSaving(true);
    const list = getFromLocalStorage(LOCAL_STORAGE_KEYS.INWARD, []);
    ...
    const item = {
      ...
      note: note.trim(),
      document: doc, // base64 data URL
    };
    list.unshift(item);
    saveToLocalStorage(LOCAL_STORAGE_KEYS.INWARD, list);
    setItems(list);
    setSaving(false);
    setToast('Saved');
    ...
    <div className="space-y-6">
      <div className="p-6 bg-card rounded-xl border-2 border-border/40 shadow-retro">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm text-muted-foreground mb-1">From Office</label>
      ...
      <div className="p-4 bg-card rounded-xl border-2 border-border/40 shadow-retro">
        <h2 className="font-semibold">Recent Inward</h2>
            ...
            filtered.slice(0,20).map((it)=>(
              <div key={it.id} className="flex items-center justify-between border border-border/40 rounded-md p-2 bg-card">
                <div>
                  ...
                  <div className="text-muted-foreground">From: {it.fromOffice} • {format(new Date(it.date),'dd MMM yyyy')}</div>
                  {(it.subject || it.note) && (
