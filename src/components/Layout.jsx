...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-gray-100 text-gray-800">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 transform bg-gray-900 overflow-y-auto transition-transform duration-300 lg:translate-x-0 lg:static ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center gap-2 px-6 py-6">
          ...
          <span className="text-2xl font-semibold text-white">My Desk</span>
        </div>
        <nav className="mt-4">
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-6 py-2 text-left transition ${active ? 'text-gray-100 bg-gray-700/30' : 'text-gray-400 hover:text-gray-100 hover:bg-gray-700/30'}`}
              >
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-100 text-gray-900">
        {/* Header */}
        <header className="flex items-center justify-between px-6 py-4 bg-white border-b-4 border-brand">
          <div className="flex items-center">
            <button aria-label="Open sidebar" onClick={() => setSidebarOpen(true)} className="text-gray-600 focus:outline-none lg:hidden">
            ...
            <div className="relative mx-4 lg:mx-0">
              <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                <svg className="w-5 h-5 text-gray-500" viewBox="0 0 24 24" fill="none"><path d="M21 21L15 15M17 10C17 13.866 13.866 17 10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </span>
              <input className="w-32 sm:w-64 pl-10 pr-4 py-2 rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand" type="text" placeholder="Search" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={notifRef}>
              <button aria-label="Open notifications" onClick={() => setNotifOpen(!notifOpen)} className="relative text-gray-600 hover:text-gray-900 transition">
                ...
                {dueToday.length > 0 && (
                  <span className="absolute -top-1 -right-1 inline-flex items-center justify-center w-5 h-5 text-xs rounded-full bg-red-500 text-white">{dueToday.length}</span>
                )}
              </button>
              {notifOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl z-10 border animate-slide-down">
                  <div className="px-4 py-3 border-b font-medium">Today</div>
                  <div className="max-h-72 overflow-auto">
                    {dueToday.length === 0 ? (
                      <div className="px-4 py-3 text-sm text-gray-600">No pending tasks for today</div>
                    ) : (
                      dueToday.map(t => (
                        <div key={t.id} className="px-4 py-2 text-sm border-b last:border-0">
                          <div className="text-gray-800">{t.title}</div>
                          <div className="text-gray-500">Due: {format(new Date(t.dueDate), 'HH:mm, dd MMM')}</div>
                        </div>
                      ))
                    )}
                  </div>
                  <div className="px-4 py-2"><Link to="/tasks" className="text-indigo-600 hover:underline">Open Tasks</Link></div>
                </div>
              )}
            </div>
            ...
            <div className="relative" ref={profileRef}>
              <button onClick={() => setProfileOpen(!profileOpen)} className="block w-8 h-8 overflow-hidden rounded-full border bg-white">
                {(() => {
                  ...
                  return <div className="w-full h-full flex items-center justify-center text-sm font-semibold bg-indigo-100 text-indigo-700">{initial}</div>;
                })()}
              </button>
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-xl z-10 animate-slide-down">
                  <Link to="/profile" className="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">Profile</Link>
                  <Link to="/login" className="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">Logout</Link>
                </div>
              )}
            </div>
          </div>
